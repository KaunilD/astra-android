...
public class OpenNIHelper {
	...
	private final int OB_END_PID = 0x0410;
	...
	public void requestDeviceOpen(String uri, DeviceOpenListener listener) {
		...
		for (int i =0; i< deviceInfos.size(); i++){
	         if(deviceInfos.get(i).getUri().equals(uri)){
				 int pid = deviceInfos.get(i).getUsbProductId();
				 int vid = deviceInfos.get(i).getUsbVendorId();
				 usbDevice = getUsbDevice(pid, vid);
	...
	public UsbDevice getUsbDevice(int pid, int vid) {
		UsbManager manager = (UsbManager) mAndroidContext.getSystemService(Context.USB_SERVICE);
		HashMap<String, UsbDevice> deviceList = manager.getDeviceList();
		Iterator<UsbDevice> iterator = deviceList.values().iterator();
